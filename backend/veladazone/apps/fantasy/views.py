...
from .serializers import FantasyLeagueSerializer, LeagueDetailSerializer
...
class FantasyLeagueViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(FantasyLeagueSerializer(league).data, status=status.HTTP_201_CREATED)
    ...
    def join(self, request):
        ...
        try:
            ...
            return Response({'error': 'Código de invitación no válido'}, status=status.HTTP_404_NOT_FOUND)

        _, created = LeagueMember.objects.get_or_create(league=league, user=request.user)
        if not created:
            return Response({'error': 'Ya eres miembro de esta liga'}, status=status.HTTP_400_BAD_REQUEST)
