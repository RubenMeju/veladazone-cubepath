...
class PredictionViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            fight = Fight.objects.select_related('fighter1', 'fighter2', 'edition').get(id=fight_id)
            winner = Fighter.objects.get(id=winner_id)
        except (Fight.DoesNotExist, Fighter.DoesNotExist):
            return Response({'error': 'Combate o luchador no encontrado'}, status=status.HTTP_404_NOT_FOUND)

        if winner not in [fight.fighter1, fight.fighter2]:
            return Response({'error': 'El luchador no participa en este combate'}, status=status.HTTP_400_BAD_REQUEST)

        opponent = fight.fighter2 if winner == fight.fighter1 else fight.fighter1
        ai_comment = generate_ai_comment(winner.name, opponent.name, fight.edition.number)
        ...
        return Response(PredictionSerializer(prediction).data, status=status.HTTP_201_CREATED if created else status.HTTP_200_OK)
    ...
    def leaderboard(self, request):
        ...
        ).filter(total__gt=0).order_by('-correct', '-total')[:10]
    ...
    def community_stats(self, request):
        ...
        for fight in fights:
            ...
            result.append({
                'fight_id': fight.id,
                'fighter1_pct': round((f1_votes / total) * 100) if total > 0 else 50,
                'fighter2_pct': round((f2_votes / total) * 100) if total > 0 else 50,
                'total_votes': total
            })
